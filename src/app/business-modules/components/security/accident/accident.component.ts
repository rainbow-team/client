...
export class AccidentComponent implements OnInit {
  ...
  search() {
    ...
    if (this.uminePlaceStatusTypeIds.length > 0) {
      option.conditions.push({
        key: 'uminePlaceStatusTypeIds',
        value: [this.uminePlaceStatusTypeIds]
      });
    }
  ...
  show(item) {
    if (this.servicedepartId) {
      this.router.navigate(
        ['/searchShow/integratedAuery/securityaccidentAdd'],
        {
          queryParams: {
            id: item.id,
            isShow: true,
  ...
  delete() {
    if (this.selectId) {
      this.accidentSecuritySercice
        .deleteAccidentSecurityById(this.selectId)
        .subscribe(res => {
          if (res.code == 200) {
  ...
  exportAccidentSecurity() {
    ...
    let url =
      ...
      '/accidentsecurity/exportAccidentSecurity?depart=' +
      this.depart +
      '&fac=' +
      this.fac +
      '&facStatusTypeIds=' +
      this.facStatusTypeIds +
      '&uminePlaceStatusTypeIds=' +
      this.uminePlaceStatusTypeIds +
      '&name=' +
      this.name +
      '&start_date=' +
